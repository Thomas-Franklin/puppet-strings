...
# Implements the handler for Puppet transport schemas written in Ruby.
class PuppetStrings::Yard::Handlers::Ruby::TransportHandler < PuppetStrings::Yard::Handlers::Ruby::Base
  ...
  namespace_only
  handles method_call(:register_transport)
  ...
  process do
    ...
    object = PuppetStrings::Yard::CodeObjects::TransportSchema.new(schema['name'])
    register object
    ...
    if docstring
      ...
      log.warn "Missing a description for Puppet schema type '#{object.name}' at #{statement.file}:#{statement.line}."
    end
  ...
  def populate_type_data(object, schema)
    return if schema['connection_info'].nil?

    schema['connection_info'].each do |name, definition|
  ...
  def create_parameter(name, definition)
    parameter = PuppetStrings::Yard::CodeObjects::TransportSchema::Parameter.new(name, definition['desc'])
    set_values(definition, parameter)
    parameter
  end

  def create_property(name, definition)
    property = PuppetStrings::Yard::CodeObjects::TransportSchema::Property.new(name, definition['desc'])
  ...
  def set_values(definition, object)
    ...
    log.warn "Object is: #{object.data_type}"
  end
end
